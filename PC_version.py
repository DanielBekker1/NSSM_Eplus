...
X_columns = ['zn_soft1_temp', 'air_loop_fan_mass_flow']  # replace with relevant columns
U_columns = ['Indoor_CO2_zn0']  # replace with relevant control columns
...
class state_observer(nn.Module):
     def __init__(self, nx, ny, nsteps, n_hidden = 80, n_layers = 3):
          super(state_observer, self).__init__()
          self.nx = nx
          self.ny = ny
          self.in_features = ny*nsteps
...
#Hyper parameters
          ...
          self.f0 = blocks.MLP(
               ...
               nonlin = nn.ReLU,
               hsizes=[self.n_hidden] * self.n_layers
          )
...
data_retriever = SimulationDataRetrieval()
...
# %% Constraints + losses:
x = variable("X")               #Translate the state variables into a "state" the Neuromancer can understand
xhat = variable('xn')[:, :-1, :]
...
for i in range(iterations):
     print(f'traning {data_retriever.nsteps} objective')
     best_model = trainer.train()
     trainer.model.load_state_dict(best_model)
     data_retriever.nsteps *= 2

     train_loader, dev_loader, test_data, nx, nu, nd, ny = \
        get_data_boptest(data_retriever)
...
#code below should save the initialsation parameters of the model 
# with open('nssm_model_params.pkl', 'wb') as f:
#      pickle.dump([nx, nu, nd, ny], f)
