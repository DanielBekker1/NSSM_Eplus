...
from Control_strategy import nx, nu, nd, U_min, U_max, nsteps, mean_d, std_d, mean_xn, std_xn
...
# Should be same varialbes as obersevation_function
        ...
        self.fan_electric_power = None  # W                 # self nr. 2
        self.fan_mass_flow = None   #kg/s                   # self nr. 3
        
        self.CO2_indoor_zn0 = None #ppm                     # self nr. 4
        self.CO2_indoor_zn1 = None #ppm                     # self nr. 5
    ...
    def observation_function(self):
        ...
        if self.time < datetime.datetime.now():
            ...
            self.fan_mass_flow = var_data[3]                    # kg/s
            self.CO2_in_zn0_con = var_data[4]                      # ppm
            self.CO2_in_zn1_con = var_data[5]                      # ppm
...
# Define thresholds - Can find example in model_test.py.
    ...
    def get_state(self):
        ...
        self.CO2_in_zn0_con,
        self.CO2_in_zn1_con,
        self.fan_electric_power,
        ]
        normalized_xn = (torch.tensor(real_time_xn, dtype=torch.float32) - mean_xn) / std_xn
        xn_tensor = torch.tensor(normalized_xn, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
fig, (ax1, ax2) = plt.subplots(ncols=3, figsize=(12, 12))  # Remember to change the ncols, number ax and figsize.
