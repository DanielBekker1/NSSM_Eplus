...
X_columns = ['zn_soft1_temp', 'air_loop_fan_mass_flow']  # should contain relevant columns
U_columns = ['Indoor_CO2_zn0']  # Should contain relevant control columns
...
for row, (t1, t2, label) in enumerate(zip(true_traj, pred_traj, labels)):
    if nx > 1:
        ...
        axe = ax
    axe.set_ylabel(label, rotation=0, labelpad=20, fontsize=front_size)
    axe.plot(t1, 'c', linewidth=4.0, label='True')
    axe.plot(t2, 'm--', linewidth=4.0, label='Pred')
    axe.tick_params(labelbottom=False, labelsize=front_size)
axe.tick_params(labelbottom=True, labelsize=figsize)
axe.legend(fontsize=front_size)
ax[-1].plot(input_traj, 'c', linewidth=4.0, label='inputs')
ax[-1].legend(fontsize=front_size)
ax[-1].set_xlabel('$time$', fontsize=front_size)
ax[-1].set_ylabel('$u$', rotation=0, labelpad=20, fontsize=front_size)
