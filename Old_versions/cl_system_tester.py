...
def plots(CO2_trajectories, desired_CO2_setpoint, P_fan_trajectories, denorm_trajectories, d):
    ...
    cumulative_cost = torch.cumsum(P_fan_mean * (electricity_price_mean), dim=0)
...
def plot_predicted_vs_test(denorm_trajectories, denorm_test_data):
    ...
    pred_xn = denorm_trajectories["xn"]
    test_xn = denorm_test_data["xn"]
    ...
    plt.subplot(2, 1, 1)
    plt.plot(test_xn[0, :, 2], linestyle="--", color="red")  # Test CO₂
    plt.plot(pred_xn[0, :, 2], linestyle="-", color="blue")  # Predicted CO₂
    plt.xlabel("Steps")
    plt.ylabel("CO₂ Concentration (ppm)")

    # Plot for control inputs (Fan Speed)
    plt.subplot(2, 1, 2)
    plt.plot(pred_u[0, :, 0], linestyle="-", color="blue")  # Predicted Fan Speed
    plt.xlabel("Steps")
